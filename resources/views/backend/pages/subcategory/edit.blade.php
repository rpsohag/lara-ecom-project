            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.subcategory.update', $subcategory->id) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                        <div class="form-group  row"><label class="col-sm-2 col-form-label">SubCategory Name</label>
                            <div class="col-sm-10">
                            <input type="text"  class="form-control @error('subcategory_name') is-invalid @enderror" value="{{ $subcategory->subcategory_name }}" name="subcategory_name">
                        ...
                        <div class="form-group  row"><label class="col-sm-2 col-form-label">Select Category</label>
                            ...
                            @foreach ($categories as $scat_data )
                                 <option @if($scat_data->id == $subcategory->cat_id) selected @endif value="{{ $scat_data->id }}">{{ $scat_data->category_name }}</option>
                            @endforeach
                        ...
                        <div class="form-group  row"><label class="col-sm-2 col-form-label">Active SubCateogry</label>
                            <div class="col-sm-10">
                                <input type="checkbox" name="status" value="1" {{ $subcategory->status == 1 ? 'checked': '' }}>
                            </div>
                        </div>
